...
export const WipLimitModal: React.FC<WipLimitModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="bg-[#13151f] border border-amber-500/40 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden ring-1 ring-amber-500/30">
        {/* Modal Header */}
        <div className="px-5 py-4 border-b border-zinc-800 bg-amber-500/10 flex items-center justify-between">
        ...
        <div className="p-5 space-y-4 text-xs text-zinc-300">
          <p className="leading-relaxed">
            You are attempting to bring <span className="font-semibold text-white">"{pendingTask.title}"</span> into <span className="text-amber-300 font-mono">In Progress</span>, but you already have <span className="font-semibold text-amber-300">{wipLimit} active tasks</span>.
          </p>
          ...
          <div className="bg-zinc-900/80 border border-zinc-800 rounded-xl p-3 space-y-2">
            ...
            {doingTasks.map((t) => (
              <div
                key={t.id}
                className="flex items-center justify-between gap-2 p-2 rounded-lg bg-zinc-950/70 border border-zinc-800/80"
              >
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    ...
                    className="flex items-center gap-1 text-[10px] font-medium bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-2 py-1 rounded transition"
          ...
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-2.5 text-[11px] text-amber-200/80 flex items-start gap-2">
        ...
        {/* Footer */}
        <div className="px-5 py-3 border-t border-zinc-800 bg-zinc-950/60 flex items-center justify-between text-xs">
          <button
            onClick={() => onForceProceed(pendingTask)}
            className="text-[11px] text-zinc-500 hover:text-amber-400 underline transition"
          >
